...
export default class SpenseController {
  ...
  static async getPeriodicallyID(dayID) {
    let { day, month, year } = getDateFromID(dayID);
    return {
  ...
  static async insert(spense, isChecked = false) {
    ...
    console.log("before insert page");
  ...
  static async insert_total(spense, isChecked = false) {
    ...
    let annualType_index = total.typeList.findIndex(
      val => val.Id == spense.typeID
    );
    if (annualType_index == -1) {
      ...
      total.typeList[annualType_index].total =
        total.typeList[annualType_index].total + spense.price;
    ...
    if (monthly_index == -1) {
      ...
      total.monthlyList[monthly_index].total =
        total.monthlyList[monthly_index].total + spense.price;

      let monthlyType_index = total.monthlyList[
        monthly_index
      ].typeList.findIndex(val => val.Id == spense.typeID);
      if (monthlyType_index == -1) {
        ...
        total.monthlyList[monthly_index].typeList[monthlyType_index].total =
          total.monthlyList[monthly_index].typeList[monthlyType_index].total +
          spense.price;
        total.monthlyList[monthly_index].typeList[
          monthlyType_index
        ].spenseList.push({
          Id: spense._id,
          price: spense.price,
    ...
    let daily_index = total.monthlyList[monthly_index].dailyList.findIndex(
    ...
    if (daily_index == -1) {
      ...
      total.monthlyList[monthly_index].dailyList[daily_index].total =
        total.monthlyList[monthly_index].dailyList[daily_index].total +
        spense.price;

      let dailyType_index = total.monthlyList[monthly_index].dailyList[
        daily_index
      ].typeList.findIndex(val => val.Id == spense.typeID);
      if (dailyType_index == -1) {
        ...
        total.monthlyList[monthly_index].dailyList[daily_index].typeList[
          dailyType_index
        ].total =
          total.monthlyList[monthly_index].dailyList[daily_index].typeList[
            dailyType_index
  ...
  static async update(spense, isChecked = false) {
    let { dayID, monthID, yearID } = await SpenseController.getPeriodicallyID(
      spense.dayID
    );
    ...
    let annualSpense_index = total.typeList[
      annualType_index
    ].spenseList.findIndex(val => val.Id == spense._id);
    ...
    let oldPrice =
      total.typeList[annualType_index].spenseList[annualSpense_index].price;

    total.total = total.total - oldPrice;
    total.total = total.total + spense.price;

    total.typeList[annualType_index].total =
      total.typeList[annualType_index].total - oldPrice;
    total.typeList[annualType_index].total =
      total.typeList[annualType_index].total + spense.price;
    ...
    let monthlyType_index = total.monthlyList[monthly_index].typeList.findIndex(
      val => val.Id == spense.typeID
    );
    // console.log('update monthlyType_index ',JSON.stringify(monthlyType_index))
    let monthlySpense_index = total.monthlyList[monthly_index].typeList[
      monthlyType_index
    ].spenseList.findIndex(val => val.Id == spense._id);
    // console.log('update monthlySpense_index ',JSON.stringify(monthlySpense_index))

    total.monthlyList[monthly_index].total =
      total.monthlyList[monthly_index].total - oldPrice;
    total.monthlyList[monthly_index].total =
      total.monthlyList[monthly_index].total + spense.price;
    ...
    total.monthlyList[monthly_index].typeList[monthlyType_index].spenseList[
      monthlySpense_index
    ] = {
      Id: spense._id,
      price: spense.price,
    ...
    let dailySpense_index = total.monthlyList[monthly_index].dailyList[
      daily_index
    ].typeList[dailyType_index].spenseList.findIndex(
      val => val.Id == spense._id
    );

    total.monthlyList[monthly_index].dailyList[daily_index].total =
      total.monthlyList[monthly_index].dailyList[daily_index].total - oldPrice;
    total.monthlyList[monthly_index].dailyList[daily_index].total =
      total.monthlyList[monthly_index].dailyList[daily_index].total +
      spense.price;
  ...
  static async delete(spense) {
    ...
    total.monthlyList[monthly_index].typeList[
      monthlyType_index
    ].spenseList.splice(monthlySpense_index, 1);
    ...
    total.monthlyList[monthly_index].dailyList[daily_index].typeList[
      dailyType_index
    ].spenseList.splice(dailySpense_index, 1);
